...
const detailedHelp = {
    ...
    diff: block([
        ...
        '*Inline counterpart:* `deriv(...)` / `integ(...)` are documented in `!help helpers`',
        '',
        '*Options*',
        ...
        '- `dep:y` — dependent variable (specifies implicit differentiation)',
        ...
        '- `!diff x^2 + y^2 = 4 dep:y` (implicit, dy/dx)',
        '- `!diff x^2 + y^2 = 4 vars:{x:2} dep:y` (second order implicit, d^2y/dx^2)'
    ]),
    ...
    helpers: block([
        '*Inline Calculus Helpers*',
        '',
        'Use these inside other commands such as `!plot`, `!solve`, and `!desp`.',
        'They return inline values or symbolic subexpressions instead of rendering a standalone image card.',
        '',
        '*Quote-free bracket notation (Recommended)*',
        'You can write helpers using square brackets `[]` and separate arguments with commas. No quotes are needed:',
        '- `deriv[x^3, x]` (d/dx of x^3)',
        '- `deriv[x^2 + y^2 = 4, dep:y, x]` (implicit dy/dx)',
        '- `integ[x^2 + y^2, x:[0, 1], y:[0, 2]]` (double definite integral)',
        '- `integ[sin(t), t, 0, x]` (definite integral from 0 to x)',
        '',
        '*Quoted parentheses notation (Alternative)*',
        '- Write the math body as the first quoted argument: `deriv("x^3", ...)`, `integ("sin(t)", ...)`',
        '- Write each helper option as its own quoted argument: `"vars:{x:2, y}"`, `"x:[0, 1]"`',
        '- Examples: `deriv("x^2 + y^2 = 4", "dep:y", "x")` or `integ("sin(t)", "vars:t")`',
        '',
        '*`deriv` helper options*',
        '- Mirrors `!diff`: single-variable, higher-order, mixed partial, and implicit derivatives',
        '- In numeric commands like `!plot` and `!solve`, the derivative is evaluated at the current variables',
        '',
        '*`integ` helper options*',
        '- Mirrors `!int`: antiderivative-style inline use, definite integrals, double/triple integrals, and line/surface/volume integrals',
        '- Examples:',
        '- `integ[sin(t), t, 0, x]`',
        ...
        '- `!plot z = integ[cos(t)*y, t, 0, x] view:3d x:[-5, 5] y:[-5, 5]`',
        '- `!solve deriv[x^3, vars:{x:2}] - 12 = 0`',
        '- `!desp deriv[x^3*y^2, vars:{x:2, y}] = z vars:y`'
    ]),
    ...
    plot: block([
        ...
        'Inline helpers such as `deriv(...)` and `integ(...)` can appear anywhere inside the expression; see `!help helpers`.',
        '',
        '*Options*',
    ...
    solve: block([
        ...
        'Inline helpers such as `deriv(...)` and `integ(...)` are allowed inside equations; see `!help helpers`.',
        '',
        '*Important rules*',
    ...
    desp: block([
        ...
        'Inline helpers such as `deriv(...)` and `integ(...)` can also appear inside the equation.',
        '',
        '*Examples*',
        '- `!desp E = m * c^2 vars:c`',
        '- `!desp PV = n*R*T for T`',
