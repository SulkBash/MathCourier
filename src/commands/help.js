...
const detailedHelp = {
    ...
    diff: block([
        ...
        '- Omit `vars` to auto-detect the variable (falls back to `x`)',
        '',
        '*Examples*',
        '- `!diff x^2 * sin(x)`',
        '- `!diff x^3 * y^2 vars:x`',
    ...
    plot: block([
        ...
        '',
        '*kind: — 2D plot types*',
        '- `kind:parametric` — parametric curve; body is `(fx(t), fy(t))`, supply a parameter range',
        '- `kind:polar` — polar curve; body is `r = f(theta)` or just `f(theta)`, supply a `theta` range',
        '- `kind:vector` — 2D vector field; body is `(fx, fy)`, supply `x` and `y` ranges',
        '',
        '*kind: — 3D plot types* (all require `view:3d`)',
        '- `kind:curve` — parametric 3D curve; body is `(fx(t), fy(t), fz(t))`, supply one parameter range',
        '- `kind:surface` — parametric 3D surface; body is `(fx, fy, fz)`, supply two parameter ranges',
        '- `kind:vector` — 3D vector field; body is `(fx, fy, fz)`, supply `x`, `y`, and `z` ranges',
        '',
        '*Examples*',
        '- `!plot y = sin(x) x:[-10, 10] y:[-2, 2]`',
    ...
    kind: block([
        ...
        '',
        '*For !plot — 2D kinds*',
        '- (omit) — explicit `y = f(x)` or implicit equation containing `=` (auto-detected)',
        ...
        '- (omit) — explicit surface `z = f(x,y)` or implicit `f(x,y,z) = c` (auto-detected)',
