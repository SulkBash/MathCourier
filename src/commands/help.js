...
const detailedHelp = {
    ...
    helpers: block([
        ...
        '- Write each helper option as its own quoted argument: `"vars:{x:2, y}"`, `"x:[0, 1]"`, `"kind:line"`',
        '- Legacy forms still work: `deriv("x^3", "x", x)` and `integ("sin(t)", "t", 0, x)`',
        '',
        '*`deriv(...)`*',
        '- Mirrors `!diff`: single-variable, higher-order, and mixed partial derivatives',
        '- If you omit `vars`, one variable is auto-detected; with several variables it prefers `x` when present',
        '- In numeric commands like `!plot` and `!solve`, the derivative is evaluated at the current command variables',
        '- Examples:',
        '- `deriv("x^3", "x", x)`',
        ...
        '- `"vars:x"` is the inline antiderivative form; in numeric contexts it uses the current variable value as the upper bound and `0` as the reference lower bound',
        '- Explicit ranges such as `"x:[0, 1]"` or `"t:[0, x]"` work inline too',
        '- Field-integral options match `!int`: `"kind:line"`, `"kind:surface"`, `"kind:volume"`, plus `"param:{...}"` when needed',
        '- Examples:',
        '- `integ("sin(t)", "vars:t")`',
